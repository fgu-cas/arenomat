...
/**
 * Handles a ENCODER_DATA response and emits the 'pin-state-'+n event where n is the pin number
 * @private
 * @param {Board} board the current arduino board we are working with.
 */

SYSEX_RESPONSE[ENCODER_DATA] = function (board) {
    var directionMask = 0x40; // B01000000
    var channelMask   = 0x3F; // B00111111 
    var positions = [];
    for (var i = 2; i < board.currentBuffer.length - 1; i+=5) {
        if (board.currentBuffer[i] === END_SYSEX) {
            continue;
        }
        var encoderNum = board.currentBuffer[i] & channelMask;
        var directionValue = board.currentBuffer[i] & directionMask;
        var direction = directionValue === 0x00 ? +1 : -1;
    
        var valueIn7Bytes = [
            board.currentBuffer[i+4],
            board.currentBuffer[i+3],
            board.currentBuffer[i+2],
            board.currentBuffer[i+1]
        ];

        var position = direction * Encoder7Bit.intFrom7Bytes(valueIn7Bytes);
        var enc = {
            index: encoderNum,
            position: position
        };
        ...
        board.emit('encoder-report-'+encoderNum, position);
    }
    
    board.emit('encoders-report', positions);
            ...
            if (board.currentBuffer.length === 0 && byt === 0) {
                ...
                if (board.currentBuffer.length === 3 && board.currentBuffer[0] !== START_SYSEX) {
                    ...
                    if (MIDI_RESPONSE[cmd]) {
                        ...
                        console.log("A bad serial read must have happened");
                        board.currentBuffer.length = 0;
                    }
                }
            }
...
/**
 * Asks the arduino to attach an encoder on the specified pins.
 * NOTE : for better performences, always use interrupt pins
 * @param {number} encoder index (0-4).
 * @param {number} pinA The first pin you want to use. Must be an interrupt pin.
 * @param {number} pinB The second pin you want to use. Must be an interrupt pin and different from pinA.
 */

Board.prototype.attachEncoder = function (encoderNum, pinA, pinB) {
    var data = [
        START_SYSEX,
        ENCODER_DATA,
        ENCODER_ATTACH,       
...
/**
 * Asks the arduino to report specified encoder position.
 * @param {number} encoder index (0-4).
 * @param {function} [callback]
 */

Board.prototype.reportEncoderPosition = function (encoderNum, callback) {
    var data = [
        START_SYSEX,
        ENCODER_DATA,
    ...
    this.sp.write(new Buffer(data));
    this.once('encoder-report-'+encoderNum, callback);
};

/**
 * Asks the arduino to report all encoders positions.
 * @param {number} encoder index (0-4).
 * @param {function} [callback]
 */

Board.prototype.reportEncodersPositions = function (callback) {
    var data = [
        START_SYSEX,
        ENCODER_DATA,
    ...
    this.sp.write(new Buffer(data));
    this.once('encoders-report', callback);
};

/**
 * Asks the arduino to report set specified encoder position to zero.
 * @param {number} encoder index (0-4).
 */

Board.prototype.resetEncoderPosition = function (encoder) {
    var data = [
        START_SYSEX,
        ENCODER_DATA,
...
/**
 * Asks the arduino to enable /disable automatic reports.
 * Reports are send every `SamplingInterval`. this interval 
 * can be updated using `board.setSamplingInterval(new_interval)` method.
 * @param {bool} true => enable, false => disable.
 */

Board.prototype.toggleEncodersPositionsReports = function (enable) {
    var data = [
        START_SYSEX,
        ENCODER_DATA,
        ENCODER_REPORT_AUTO,
...
/**
 * Asks the arduino to detach an encoder on the specified pins.
 * @param {number} encoder index (0-4).
 */

Board.prototype.detachEncoder = function (encoderNum) {
    var data = [
        START_SYSEX,
        ENCODER_DATA,
        ENCODER_DETACH,       
